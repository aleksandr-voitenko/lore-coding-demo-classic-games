...
- Snake, Tetris, Breakout, Minesweeper, Pong, Simon, 2048, and Asteroids replay recording with
  server-issued runs, signed-in replay saves, and profile playback for the
  latest saved run.
...
Snake, Tetris, Breakout, Minesweeper, Pong, Simon, 2048, and Asteroids record replay events
during play after the server issues a run id and seed. Signed-in players can
save the completed run from the final screen; the profile page stores one
latest replay per user and game and links back into the launcher at
`/?replay=snake`, `/?replay=tetris`, `/?replay=breakout`,
`/?replay=minesweeper`, `/?replay=pong`, `/?replay=simon`,
`/?replay=twenty-forty-eight`, or `/?replay=asteroids` for client playback.
