...
- Snake, Tetris, Breakout, Minesweeper, and 2048 replay recording with
...
Snake, Tetris, Breakout, Minesweeper, and 2048 record replay events during play
after the server issues a run id and seed. Signed-in players can save the
completed run from the final screen; the profile page stores one latest replay
per user and game and links back into the launcher at `/?replay=snake`,
