...
- `snake-replay.ts`, `tetris-replay.ts`, `breakout-replay.ts`,
  `minesweeper-replay.ts`, `pong-replay.ts`, `simon-replay.ts`,
  `twenty-forty-eight-replay.ts`, and `asteroids-replay.ts` own game-specific
  ...
  flag toggles, Pong starts/advances/paddle movement/score ticks, Simon
  phase/input events, 2048 move directions, and Asteroids
  starts/advances/control-state changes/fire events rather than video or full
  board snapshots.
